...
class BVariable(RecursivelyReferenceable, FormatDispatcher, MetabuildObject):
  ...
  def __init__(self,  doc:str, build:LabsBuild, name:str, expr:Expr|str=None):
    super().__init__()
    self.doc = doc
    self.build = build
    self.name = name
  ...
  @property
  def value(self):
    return format(self.expr, 'expanded')

  @value.setter
  def value(self, val):
    self.expr = format(Expr(val), 'expanded')
...
class BRule(UseInternal, LabsObject, FormatDispatcher, MetabuildObject):
  """
  ...
  class _Internal(object):
    ...
    def _add_buildin_variables(self, parent:BRule):
      apply(self.add_variable, (
    ...
    def get(self, key, default=None):
      r = self.builtins.get(key)
      if r is not None:
        return r
      r = self.variables.get(key)
      if r is not None :
        return r
      return default

  def __init__(self, build:LabsBuild, name:str, command:Expr|str=None):
    super().__init__(build, name)
    self._internal._add_buildin_variables(self)
  ...
  def __setattr__(self, key, val):
    ...
    if current is None :
      ...
      if isinstance(val, Decl) :
        raise TypeError(f'{self} already has a variable {key} ({current}). Cannot override it with {val}')
      current.expr = val
  ...
  def instanciate(cls, decl, build, name):
    r = cls(build, name, decl.command)
    build._register_metabuild_object(name, r)
    return r

  @classmethod
  def decl(cls, command=None):
    return cls.decl_cls(command=command)
...
class BStep(LabsObject, UseInternal, FormatDispatcher, MetabuildObject):
  """
